...
function cetak() {
	document.getElementById('row_cetak').remove();
	window.print();
}
function ask(tanya) {
	return confirm(tanya);
}
</script>
<form method="post" action="<?=site_url('jurutera/tolakkewangan'); ?>" 
onsubmit="return ask('Pasti untuk meluluskan kewangan ini?\n[Perubahan tidak dapat dipinda]');">
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="tbl_temp">
  <tr id="row_cetak">
    <td colspan="4">
    	<button type="button" name="print_1" id="print_1" onclick="cetak();">Cetak</button>
        <?php if($lo->lo_expenses == 0.00 || $lo->lo_expenses == 0) { ?>
    	<button type="submit" name="kewangan" id="kewangan">Lulus Kewangan</button>
        <?php } ?>
        <input type="hidden" name="lo_id" id="lo_id" value="<?=$lo->lo_id; ?>" />
        <input type="hidden" name="f_code" id="f_code" value="<?=$lo->f_code; ?>" />
    &nbsp;</td>
  </tr>
  <tr>
          ...
          <tr>
            ...
            <td colspan="2" align="center" valign="top"><table width="100%" border="0" cellpadding="5" cellspacing="0">
              ...
              <tr>
                ...
                <td align="left" class="signature1"><strong>:
                  <?=$sess['u_fullname']; ?>
                </strong></td>
              </tr>
              <tr>
...
<input type="hidden" name="lo_expenses" id="lo_expenses" value="<?=$total+$total2; ?>" />
</form>