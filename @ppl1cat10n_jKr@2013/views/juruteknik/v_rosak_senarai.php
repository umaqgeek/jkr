...
$(document).ready(function() {
	$(".padam").click(function() {
		if(ask('Padam data ini?')) {
			...
			location.href = "<?=site_url('juruteknik/brokendelete');?>/"+id;
		}
	});
	$(".kemaskini").click(function() {
		var id = $(this).attr("idd");
		location.href = "<?=site_url('juruteknik/brokenedit');?>/"+id;
	});
	$(".nota").click(function() {
		var id = $(this).attr("idd");
		location.href = "<?=site_url('juruteknik/notaminta/1');?>/"+id;
	});
	$(".lo").click(function() {
		var id = $(this).attr("idd");
		location.href = "<?=site_url('juruteknik/brokenliv');?>/"+id;
	});
});
</script>
...
<div class="span8 offset4">
	<div class="span2">
    	<a href="<?=site_url('juruteknik/broken/1')?>"><img src="<?=base_url()?>/assets/images/gnome/PNG/64/Gnome-List-Add-64.png" />
        <br/><span style="margin-left:-20px;">Daftar Log Kerja</span></a>
    </div>
    <div class="span2">
    	<a href="<?=site_url('juruteknik/brokenexport')?>"><img src="<?=base_url()?>/assets/images/gnome/PNG/64/Gnome-Document-Save-64.png" />
...
<?php if($this->m_letter_order->is_brokenlog_exist($bl->bl_id)) { ?>
                    <button type="button" class="btn nota" idd="<?=$bl->bl_id;?>">Nota Minta</button>
<?php } else { ?>
					<button type="button" class="btn lo" idd="<?=$bl->bl_id;?>">Daftar L.I.V.</button>
<?php } ?>
